...
Redis 分布式缓存库，基于 [redis-cj](https://github.com/bambuo/redis-cj) 客户端实现 Spire 框架的 `IDistributedCache` 接口。输出类型为**静态库**（`output-type = "static"`），供其他仓颉项目作为依赖引用。

### 核心架构

```
src/
├── main.cj                          # 包入口（package cache，空 main）
└── distributed/
    ├── distributed.cj               # 包声明（package cache.distributed）
    └── redis/
...
**包层次**：`cache` → `cache.distributed` → `cache.distributed.redis`

### 核心模块

- **[RedisDistributedCache](src/distributed/redis/redis_cache.cj)** — 实现 `IDistributedCache` 接口，持有 `RedisClient` 实例。提供 `get/set/getString/setString/refresh/remove` 方法，支持无过期、相对过期、绝对过期、滑动过期四种策略。滑动过期使用独立元数据键 `{key}:__ttl__` 存储窗口值，在 `get/getString/refresh` 时自动续期。

- **[expiry_utils](src/distributed/redis/expiry_utils.cj)** — 纯函数模块，不含任何状态。核心函数 `resolveExpiry` 将 `DistributedCacheEntryOptions` 解析为 Redis `SET` 命令的 `ex/px/exAt/pxAt` 参数元组。

- **[redis_cache_test](src/distributed/redis/redis_cache_test.cj)** — 单元测试只测 `expiry_utils` 中的纯函数（无需 Redis）；集成测试需连接真实 Redis 实例，每个测试用例使用 `uniqueKey` 生成独立键名避免互相干扰。
...
以 `cache.distributed.redis` 为根包路径，外部引用写法：
